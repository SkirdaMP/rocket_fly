...
class Obstacle:
    ...
    def has_collision(self, obj_corner_row, obj_corner_column, obj_size_rows=1, obj_size_columns=1):
        ...
        if obj_corner_column + obj_size_columns < self.column:
            return has_collision(
                (self.row, self.column + 2),
                (self.rows_size, self.columns_size),
                (obj_corner_row, obj_corner_column),
                (obj_size_rows, obj_size_columns),
            )
        elif obj_corner_column > self.column + self.columns_size:
            return has_collision(
                self.get_bounding_box_corner_pos(),
                (self.rows_size, self.columns_size),
                (obj_corner_row, obj_corner_column),
                (obj_size_rows, obj_size_columns),
            )
...
async def show_obstacles_boxes(canvas, obstacles):
